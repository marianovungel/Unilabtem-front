...
import './PageSingleCompart.css'
import api from '../../services/api';
import { useLocation } from 'react-router-dom';
import Swal from 'sweetalert2';

export default function PageSingleCompart() {
    ...
    const Autorizar = async ()=>{
        try{
            await api.put("/compartilharmonitor/"+path, {
                estado: "visivel"
            })
            Swal.fire({
                position: 'center',
    ...
    const Reprovar = async ()=>{
        try{
            await api.delete("/compartilharmonitor/"+path)
            Swal.fire({
                position: 'center',
                icon: 'error',
    ...
    useEffect(()=>{
        const getPost = async ()=>{
            try{
                const res = await api.get("/compartilharmonitor/"+path)
                setPost(res.data)
